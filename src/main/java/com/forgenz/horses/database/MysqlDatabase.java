...
public class MysqlDatabase extends HorseDatabase {
    ...
    private boolean connect() {
        ...
        String connection = String.format(Locale.US, "jdbc:mysql://%s/%s", this.settings.host, this.settings.database);
        try {
    ...
    protected void importStables(List<Stable> stables) {
        HashMap<String, Integer> stableIds = new HashMap<>();
        ...
        try {
            ...
            while (result.next()) {
                try {
                    ...
                    boolean hasChest = ((type == HorseType.Mule) || (type == HorseType.Donkey)) && result.getBoolean("chested");
                    ...
                    if (items != null) {
                        horseData.setItems((ItemStack[]) items.toArray(new ItemStack[0]));
                        items.clear();
                    }
    ...
    public void saveHorse(PlayerHorse horse) {
        ...
        if (horse.getId() == -1) {
            try {
                String query = String.format(Locale.US, "INSERT INTO `Horses` (`stableid`, `stablegroup`, `name`, " +
                                "`type`, `lastdeath`, `maxhealth`, `health`, `speed`, `jumpstrength`, `chested`, `inventory`)" +
                                " VALUES ('%d', '%s', '%s', '%s', '%d', '%f', '%f', '%f', '%f', '%d', '%s')", horse.getStable().getId(),
                        horse.getStable().getGroup(), name, type.toString(), lastDeath, maxhealth, health, speed,
                        jumpstrength, chested ? 1 : 0, inventoryString);
                ...
                getPlugin().severe("Failed to insert the player %s's horse '%s' into the database", e,
                        new Object[]{horse.getStable().getOwner(), horse.getName()});
            }

        } else {
            try {
                String query = String.format(Locale.US, "UPDATE `Horses` SET `name`='%s', `type`='%s'," +
                                " `lastdeath`='%d', `maxhealth`='%f', `health`='%f', `speed`='%f', `jumpstrength`='%f'," +
                                " `chested`='%d', `inventory`='%s' WHERE `id`='%d'", name, type.toString(), lastDeath,
                        maxhealth, health, speed, jumpstrength, chested ? 1 : 0, inventoryString, horse.getId());

                stmt.executeUpdate(query);
            } catch (SQLException e) {
                getPlugin().severe("Failed to update the player %s's horse '%s' in the database", e,
                        new Object[]{horse.getStable().getOwner(), horse.getName()});
            }
        }
    }
    ...
    private static class MysqlSettings extends AbstractConfig {
        ...
        protected MysqlSettings(Horses plugin) {
            ...
            this.host = getAndSet("Host", "localhost", String.class);
            this.database = getAndSet("Database", "default", String.class);

            this.user = getAndSet("User", "root", String.class);
            this.password = getAndSet("Password", "", String.class);
